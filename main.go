...
var (
	...
	labelPatchExistingLabels = `[{"op":"add","path":"/metadata/labels/thisisanewlabel", "value":"toReplace"}]`
...
func mutatePods(receivedAdmissionReview v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	log.Info("The pod sa %v", string(pod.Spec.ServiceAccountName))

	if len(pod.ObjectMeta.Labels) > 0 {
		reviewResponse.Patch = []byte(str.ReplaceAll(labelPatchExistingLabels, "toReplace", pod.Spec.ServiceAccountName))
	} else {
		reviewResponse.Patch = []byte(str.ReplaceAll(labelPatchExistingLabels, "toReplace", pod.Spec.ServiceAccountName))
	}
...
func serve(w http.ResponseWriter, r *http.Request) {
	...
	log.Info("The request %v", string(requestDump))
	...
	log.Info("The receivedAdmissionReview %+v", receivedAdmissionReview)
	deserializer := codecs.UniversalDeserializer()
